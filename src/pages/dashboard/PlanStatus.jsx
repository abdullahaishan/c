...
// ============================================
// مكون عرض سجل التشخيص في الصفحة
// ============================================
const DebugPanel = ({ logs }) => {
  ...
  if (!isVisible) {
    return (
      <button
        ...
        className="fixed bottom-4 left-4 z-50 bg-yellow-600 text-white px-3 py-1 rounded-lg text-xs opacity-50 hover:opacity-100 transition-opacity"
      >
        🔧 إظهار التشخيص
      </button>
    )
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 bg-black/90 text-white p-4 rounded-xl border border-yellow-500/50 max-w-md max-h-96 overflow-auto text-xs font-mono">
      <div className="flex items-center justify-between mb-2 sticky top-0 bg-black/90 pb-2">
        <h4 className="text-yellow-400 font-bold">🔍 سجل التشخيص</h4>
        <button
...
const PlanStatus = () => {
  ...
  const addDebugLog = (message) => {
    const timestamp = new Date().toLocaleTimeString('ar', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
    setDebugLogs(prev => [...prev, { timestamp, message }].slice(-20)) // آخر 20 رسالة فقط
  }
  ...
  // ✅ useEffect للتشخيص
  useEffect(() => {
    addDebugLog('=== تشخيص الباقة ===')
    addDebugLog(`user object موجود: ${user ? '✅' : '❌'}`)
    addDebugLog(`user?.plan_id: ${user?.plan_id || 'غير موجود'}`)
    addDebugLog(`userPlanId (من DB): ${userPlanId || 'غير موجود'}`)
    addDebugLog(`currentPlan: ${currentPlan?.name || 'غير موجود'}`)
    addDebugLog(`currentPlan ID: ${currentPlan?.id || 'غير موجود'}`)
  }, [user, userPlanId, currentPlan])
  ...
  const fetchAllData = async () => {
    setLoading(true)
    addDebugLog('جاري جلب البيانات...')
    
    try {
      // 1️⃣ جلب جميع الباقات
      addDebugLog('جلب جميع الباقات...')
      ...
      if (plansError) {
        addDebugLog(`❌ خطأ في جلب الباقات: ${plansError.message}`)
        throw plansError
      }
      
      setAllPlans(plans || [])
      addDebugLog(`✅ تم جلب ${plans?.length || 0} باقة`)

      // 2️⃣ جلب plan_id من قاعدة البيانات مباشرة
      addDebugLog('جلب plan_id من قاعدة البيانات...')
      ...
      if (userError) {
        addDebugLog(`❌ خطأ في جلب user plan: ${userError.message}`)
        throw userError
      }
      
      addDebugLog(`✅ user plan_id من DB: ${userData?.plan_id || 'غير موجود'}`)
      
      // 3️⃣ تخزين plan_id
      if (userData?.plan_id) {
        setUserPlanId(userData.plan_id)
        addDebugLog(`✅ تم تخزين userPlanId = ${userData.plan_id}`)
        
        // 4️⃣ جلب تفاصيل الباقة
        addDebugLog(`جلب تفاصيل الباقة رقم ${userData.plan_id}...`)
        ...
        if (planDetails) {
          ...
          addDebugLog(`✅ الباقة الحالية: ${planDetails.name} (ID: ${planDetails.id})`)
        } else {
          addDebugLog(`❌ لم يتم العثور على تفاصيل الباقة`)
        }
      } else {
        addDebugLog(`⚠️ user.plan_id غير موجود، استخدام القيمة الافتراضية 1`)
        setUserPlanId(1)
      }

      // 5️⃣ جلب الاستخدام الحالي
      addDebugLog('جلب الاستخدام الحالي...')
      const content = await statsService.getContentStats(user.id)
      setUsage(content?.counts || {})
      addDebugLog('✅ تم جلب الاستخدام الحالي')

    } catch (error) {
      addDebugLog(`❌ خطأ عام: ${error.message}`)
      console.error('Error fetching data:', error)
    } finally {
      setLoading(false)
      addDebugLog('✅ انتهى تحميل البيانات')
    }
  }
  ...
  const loadExchangeRates = async () => {
    ...
    try {
      ...
      addDebugLog('✅ تم تحديث أسعار الصرف')
    } catch (error) {
      addDebugLog(`❌ خطأ في تحديث الأسعار: ${error.message}`)
    } finally {
  ...
  const detectCountry = async () => {
    ...
    try {
      ...
      addDebugLog(`✅ تم كشف الدولة: ${data.country_name} (${data.currency})`)
    } catch (error) {
      addDebugLog(`❌ خطأ في كشف الدولة: ${error.message}`)
    } finally {
  ...
  // ✅ دالة isCurrentPlan المعدلة - الأهم هنا
  const isCurrentPlan = (planId) => {
    ...
    if (userPlanId) {
      ...
      result = currentId === targetId
      source = `userPlanId (${currentId})`
    }
    // fallback إلى user?.plan_id
    else if (user?.plan_id) {
      const currentId = Number(user.plan_id)
      result = currentId === targetId
      source = `user.plan_id (${currentId})`
    }
    
    addDebugLog(`🔍 مقارنة الباقة ${planId}: ${result ? '✔️ حالية' : '✖️ ليست حالية'} (المصدر: ${source})`)
    return result
  }
  ...
  return (
    <>
      ...
      {/* ✅ لوحة التشخيص - تظهر في أسفل يسار الصفحة */}
      <DebugPanel logs={debugLogs} />
    </>
  )
}
