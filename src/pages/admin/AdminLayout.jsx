...
const AdminLayout = () => {
  ...
  // ✅ التحقق من صلاحية الأدمن - التوجيه الصحيح
  if (!admin) {
    console.log('❌ No admin found, redirecting to /admin/login')
    return <Navigate to="/admin/login" replace />  // ← تم التصحيح!
  }
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-400 capitalize">{admin?.role || 'Administrator'}</p>
            </div>
      ...
      <div className="lg:ml-64">
        ...
        <header className="h-16 bg-white/5 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-4 lg:px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-gray-400">Welcome, {admin?.full_name?.split(' ')[0] || admin?.email}</span>
          </div>
        </header>
