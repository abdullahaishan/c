...
const AdminLayout = () => {
  ...
  useEffect(() => {
    if (!admin) {
      setError('لم يتم تحميل بيانات الأدمن')
    }
  }, [admin])

  // تحميل عدد الإشعارات مع try/catch
  useEffect(() => {
    const loadNotifications = async () => {
      ...
      try {
        ...
        if (!adminNotificationService?.getUnreadNotifications) {
          console.warn('adminNotificationService غير متوفر')
          return
        }
        ...
        // لا نوقف التطبيق بسبب الإشعارات
      }
    }
  ...
  // التحقق من admin
  if (!admin) {
    // نعرض رسالة خطأ بدلاً من التوجيه المباشر
    return (
      <div className="min-h-screen bg-[#030014] flex items-center justify-center">
        <div className="text-center p-8 bg-white/5 rounded-2xl border border-red-500/20">
          <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" />
          <h2 className="text-2xl text-white mb-2">خطأ في تحميل البيانات</h2>
          <p className="text-gray-400 mb-6">لم يتم العثور على بيانات الأدمن</p>
          <Link
            to="/admin/login"
  ...
  return (
    <div className="min-h-screen bg-[#030014]">
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:right-0 lg:w-64 lg:block bg-white/5 backdrop-blur-xl border-l border-white/10">
        ...
        {/* Admin Info - مع التحقق من وجود admin */}
        <div className="p-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <img
              src={admin?.profile_image || '/default-avatar.png'}
              alt={admin?.full_name || 'أدمن'}
              className="w-10 h-10 rounded-full object-cover"
            />
            <div className="flex-1">
              <p className="text-sm font-medium text-white">{admin?.full_name || 'أدمن'}</p>
      ...
      <div className="lg:mr-64">
        ...
        <header className="h-16 bg-white/5 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-4 lg:px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-gray-400">مرحباً، {admin?.full_name?.split(' ')[0] || 'أدمن'}</span>
          </div>
        </header>
